...
export function ColorVisualizerPage() {
  ...
  const [tab, setTab] = useState<'wheel' | 'presets' | 'pantone'>('wheel')
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            {!imageUrl ? (
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
                {/* Example wall photos — carregadas para testar imediatamente */}
                <div>
                  <p className="text-xs font-medium text-gray-500 mb-2 flex items-center gap-1.5">
                    <Sparkles className="w-3.5 h-3.5 text-brand" /> Experimente com uma foto de exemplo
                  </p>
                  <div className="grid grid-cols-3 gap-3">
                    {[
                      { url: '/parede1.png', label: 'Sala clara' },
                      { url: '/parede2.png', label: 'Sala ampla' },
                      { url: 'https://images.unsplash.com/photo-1586023492125-27b2c045efd7?w=900&q=85', label: 'Quarto' },
                    ].map(({ url, label }) => (
                      <motion.button
                        key={url}
                        whileHover={{ scale: 1.02, boxShadow: '0 8px 24px rgba(0,0,0,0.12)' }}
                        whileTap={{ scale: 0.98 }}
                        onClick={() => { setImageUrl(url); setImageName(label); setSaved(false); setAppliedColors([]) }}
                        className="relative rounded-2xl overflow-hidden border-2 border-gray-100 hover:border-brand transition-colors cursor-pointer group"
                        style={{ aspectRatio: '4/3' }}
                      >
                        <img src={url} alt={label} className="w-full h-full object-cover" />
                        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors flex items-end">
                          <span className="w-full text-center text-xs font-semibold text-white py-1.5 bg-black/40 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity">
                            {label}
                          </span>
                        </div>
                      </motion.button>
                ...
                <div
                  ...
                  className="border-2 border-dashed border-gray-200 rounded-2xl p-8 text-center bg-white hover:border-brand hover:bg-orange-50/30 transition-colors cursor-pointer"
                ...
                >
                  ...
                  <p className="font-semibold text-gray-800 text-sm mb-1">
                    {uploading ? 'Carregando...' : 'Ou faça upload da sua própria foto'}
                  </p>
                  <p className="text-xs text-gray-400">Arraste ou clique · JPG, PNG · Máximo 10MB</p>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
              <div className="flex border-b border-gray-100">
                {(['wheel', 'presets', 'pantone'] as const).map(t => (
                  <button key={t} onClick={() => setTab(t)}
                    className={`flex-1 py-2.5 text-xs font-medium transition-colors cursor-pointer ${tab === t ? 'text-brand border-b-2 border-brand bg-orange-50/40' : 'text-gray-500 hover:text-gray-700'}`}>
                    {t === 'presets' ? 'Cores prontas' : t === 'wheel' ? 'Cromático' : 'Pantone'}
                  </button>
                ))}
              </div>
                ...
                <AnimatePresence mode="wait">
                  {tab === 'wheel' && (
                    <motion.div key="wheel" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                      ...
                      <ColorWheel size={210} value={selectedColor} onChange={handleColorChange} />
                      <p className="text-xs text-gray-400 text-center">Clique ou arraste para selecionar a cor</p>
                    </motion.div>
                  )}
                      ...
                      <div className="grid grid-cols-5 gap-1.5 max-h-64 overflow-y-auto">
                        {PAINT_PRESETS.map(preset => (
                          <button key={preset.hex} onClick={() => handlePresetSelect(preset)} title={preset.name}
                            className={`relative aspect-square rounded-lg border-2 transition-all cursor-pointer hover:scale-105 ${
                  ...
                  {tab === 'pantone' && (
                    <motion.div key="pantone" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                      <div className="grid grid-cols-4 gap-2 max-h-72 overflow-y-auto">
                        {PANTONE_COLORS.map(p => (
                          <button
                            key={p.code || p.hex}
                            onClick={() => { setSelectedColor(p.hex); setColorName(`Pantone ${p.name}`) }}
                            title={`${p.name}${p.code ? ` · ${p.code}` : ''}`}
                            className={`relative flex flex-col rounded-xl overflow-hidden border-2 transition-all cursor-pointer hover:scale-105 ${
                              selectedColor === p.hex ? 'border-brand scale-105 shadow-md' : 'border-gray-200 hover:border-gray-400'
                            }`}
                          >
                            <div className="h-10 w-full" style={{ backgroundColor: p.hex }} />
                            <div className="px-1 py-0.5 bg-white">
                              <p className="text-[9px] font-semibold text-gray-700 leading-tight truncate">{p.name}</p>
                              {p.code && <p className="text-[8px] text-gray-400">{p.code}</p>}
                              {p.year && <p className="text-[8px] text-brand font-medium">{p.year}</p>}
                            </div>
                            {selectedColor === p.hex && (
                      ...
                      <p className="text-xs text-gray-400 text-center mt-3">
                        {PANTONE_COLORS.find(p => p.hex === selectedColor)?.name
                          ? `Pantone ${PANTONE_COLORS.find(p => p.hex === selectedColor)?.name}`
                          : 'Selecione uma cor Pantone'}
                      </p>
                    </motion.div>
                  )}
                </AnimatePresence>
