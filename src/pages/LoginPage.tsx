...
export function LoginPage() {
  ...
  async function handleEmailContinue(e: React.FormEvent) {
    ...
    const { data: check } = await supabase.functions.invoke('register-user', {
      body: { email: email.toLowerCase().trim(), checkOnly: true },
    })
  ...
  async function handleSendPasswordLink() {
    ...
    else { setSuccessMsg('Link enviado! Verifique seu email para criar sua senha.'); setStep('success') }
    setLoading(false)
  }
            ...
            <AnimatePresence mode="wait">
              ...
              {step === 'login' && (
                <motion.div key="login" initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 16 }} transition={{ duration: 0.2 }}>
                  ...
                  {/* Google-only account warning */}
                  {providers.includes('google') && !providers.includes('email') && (
                    <div className="bg-blue-50 border border-blue-100 rounded-xl px-4 py-3 mb-4 space-y-3">
                      <p className="text-xs text-blue-800 font-medium">
                        🔵 Esta conta foi criada com Google
                      </p>
                      <p className="text-[11px] text-blue-600">
                        Entre com Google ou crie uma senha para acessar também com email.
                      </p>
                      <div className="flex flex-col gap-2">
                        <button onClick={handleGoogleLogin} disabled={loading}
                          className="w-full py-2.5 bg-white border border-blue-200 rounded-xl text-sm font-semibold text-gray-700 flex items-center justify-center gap-2 cursor-pointer hover:bg-blue-50 transition-colors disabled:opacity-60">
                          <svg width="16" height="16" viewBox="0 0 24 24"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg>
                          Entrar com Google
                        </button>
                        <button type="button" onClick={handleSendPasswordLink} disabled={loading}
                          className="w-full text-xs text-blue-600 hover:text-blue-800 transition-colors text-center cursor-pointer underline underline-offset-2">
                          {loading ? 'Enviando...' : 'Criar senha para login com email →'}
                        </button>
                      </div>
                    </div>
                  )}
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input type={showPw ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)}
                          required={!providers.includes('google') || providers.includes('email')}
                          autoComplete="current-password" autoFocus={!providers.includes('google')}
