...
- Name: 10-multi-role-journey.spec.ts >> 10 — Jornada multi-perfil (cliente → pintor → clube) >> Fase 1B — Envia lead pelo chat (auth gate não aparece — já logado)
- Location: e2e/10-multi-role-journey.spec.ts:133:3
...
Call log:
  - waiting for locator('text=/Pedido enviado|Protocolo|PT-/') to be visible
