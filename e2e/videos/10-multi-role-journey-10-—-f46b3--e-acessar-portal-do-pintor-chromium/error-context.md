...
- Name: 10-multi-role-journey.spec.ts >> 10 — Jornada multi-perfil (cliente → pintor → clube) >> Fase 2C — DB: promover para painter e acessar portal do pintor
- Location: e2e/10-multi-role-journey.spec.ts:267:3
...
Locator: locator('h1, h2').filter({ hasText: /Dashboard|Solicitações|Bem-vindo|Pintor/i }).first()
Expected: visible
Timeout: 8000ms
...
Call log:
  ...
  - waiting for locator('h1, h2').filter({ hasText: /Dashboard|Solicitações|Bem-vindo|Pintor/i }).first()
